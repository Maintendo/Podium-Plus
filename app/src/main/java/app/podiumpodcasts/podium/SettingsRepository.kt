...
object SettingsKeys {
    ...
    val BEHAVIOR_AUTO_QUEUE = booleanPreferencesKey("behavior_auto_queue")
...
class SettingsRepository(val context: Context) {
    ...
    inner class Behavior {
        val autoQueue: Flow<Boolean> = dataStore.data
            .map { preferences ->
                preferences[SettingsKeys.BEHAVIOR_AUTO_QUEUE] ?: true
            }

        suspend fun setAutoQueue(enable: Boolean) = dataStore.edit { preferences ->
            preferences[SettingsKeys.BEHAVIOR_AUTO_QUEUE] = enable
        }
